...
import { cancelOrder } from '@/app/(store)/pedido/actions'
...
export function CancelOrderButton({ orderId }: Props) {
  ...
  return (
    <>
      <button
        onClick={open}
        className="inline-flex items-center gap-2 px-5 py-2.5 rounded-xl border border-error/40 text-error text-sm font-body hover:bg-error/10 transition-colors"
      >
        Cancelar pedido
      </button>
            ...
            >
              <div className="bg-card border border-rim rounded-2xl p-6 max-w-xs w-full shadow-2xl pointer-events-auto">
                ...
                <p className="font-body text-sm text-fg-2 mb-5 leading-relaxed">
                  Esta acción no se puede deshacer. ¿Segura que deseas cancelar este pedido?
                </p>
                {error && (
